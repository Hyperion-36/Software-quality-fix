    ...
    def __init__(self):
        ...
        def initTab3():  # 设置卡
            ...
            def quickOCR():  # 快捷识图设置
                ...
                self.balloon.bind(wid, '每次快捷识图将清空识别内容面板，同时省略时间等信息')
            ...
            def initScheduler():  # 计划任务设置
                ...
                okMissionNameList = list(i for i in okMissionDict.keys())
        ...
        for path in paths:  # 遍历拖入的所有路径
            if os.path.isdir(path):  # 若是目录
                if isRecursiveSearch:  # 需要递归子文件夹
                    for subDir, subFiles in os.walk(path):
                        for s in subFiles:
                            addImage(subDir+"\\"+s)
                else:  # 非递归，只搜索子文件夹一层
